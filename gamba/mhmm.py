...
LOOKBACK_SHIFT = 10  # Exclude last 10 candles
...
def calculate_indicators(df):
    try:
        # Key Fix: Shift rolling window by 10 to exclude recent 10 candles
...
def generate_signals(df):
    ...
    buy_condition = (
        (df['close'] <= df['support'] * (1 + BUFFER_PCT)) &  # Uses shifted support
        (df['fast_sma'] > df['slow_sma']) &
        (df['fisher'] > df['fisher_signal'])
    )
    
    sell_condition = (
        (df['close'] >= df['resistance'] * (1 - BUFFER_PCT)) &  # Uses shifted resistance
        (df['fast_sma'] < df['slow_sma']) &
        (df['fisher'] < df['fisher_signal'])
    )
...
def run_bot():
    ...
    print(f"Bot started - Trading {SYMBOL} using 75-period highs/lows (excludes last 10 candles)")
    
    while True:
        try:
            df = fetch_candles(SYMBOL, TIMEFRAME)
            if len(df) < (SR_WINDOW + LOOKBACK_SHIFT) * 2:  # Adjusted data check
            ...
            print(f"75-period Low (shifted): {df['support'].iloc[-1]:.3f}")
            print(f"75-period High (shifted): {df['resistance'].iloc[-1]:.3f}")
